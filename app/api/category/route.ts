...
export async function POST(request: Request) {
  ...
  try {
    const category = await authorizedFetch<CategoryRead>("/category/create", token, {
      method: "POST",
      body: formData,
    });
    return NextResponse.json({ detail: "Category created", category }, { status: 201 });
  } catch (err) {
    const status = err instanceof ApiError ? err.status : 500;
    const detail = err instanceof ApiError ? err.message : "Failed to create category";
    return NextResponse.json({ detail }, { status });
  }
}
